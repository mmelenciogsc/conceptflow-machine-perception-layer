...
class RemotePerceptionClientTest {
    ...
    fun negotiationIsBoundedAndDeclaresOnlySupportedCapabilities() {
        ...
        assertEquals(1_920, request.capabilities.maxWidth)
        assertEquals(1_080, request.capabilities.maxHeight)
        assertEquals(2L * 1_024L * 1_024L, request.capabilities.maxFrameBytes)
        assertEquals(1, request.requestedQos.maxInFlight)
        assertEquals(5, request.requestedQos.targetFramesPerSecond)
